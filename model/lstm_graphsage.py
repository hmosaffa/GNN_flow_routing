...
class SimpleGraphSAGEWithStatic(nn.Module):
    """2-layer GraphSAGE module for spatial processing with static feature integration."""
    
    def __init__(self, temporal_dim, static_dim, hidden_dim, output_dim, dropout=0.5, aggregation='mean'):
        super(SimpleGraphSAGEWithStatic, self).__init__()
...
class CombinedLSTMGraphSAGEWithStatic(nn.Module):
    """
    Combined LSTM-GraphSAGE model for spatiotemporal forecasting.
    ...
    3. 2-Layer GraphSAGE: Propagates information across spatial network with inductive learning
    ...
    Args:
        ...
        gnn_hidden_dim (int): Hidden dimension for GraphSAGE
        output_dim (int): Output dimension (default: 1)
        lstm_layers (int): Number of LSTM layers (default: 2)
        sage_aggregation (str): Aggregation method for GraphSAGE ('mean', 'max', 'add', 'lstm')
        lstm_dropout (float): Dropout rate for LSTM (default: 0.2)
        gnn_dropout (float): Dropout rate for GraphSAGE (default: 0.2)
    """
    ...
    def __init__(
        ...
        super(CombinedLSTMGraphSAGEWithStatic, self).__init__()
        ...
        self.gnn_module = SimpleGraphSAGEWithStatic(
            temporal_dim=lstm_hidden_dim,
            static_dim=static_input_dim,
            hidden_dim=gnn_hidden_dim,
            output_dim=output_dim,
            aggregation=sage_aggregation,
            dropout=gnn_dropout
        )
